...
const main = async () => {
    ...
    for (let i = 0; i < numWallet; i++) {
        try {
            ...
            let balance = await provider.getBalance(walletiWithProvider.address);
            // 计算所需要的交易费用
            ...
            if (amount > 0) {
                let tx = await walletiWithProvider.sendTransaction({
                    to: wallet.address,
                    value: amount
                });
                console.log(`第 ${i+1} 个钱包 ${walletiWithProvider.address} ETH 归集开始, 归集金额: ${ethers.formatEther(amount)}`);
                await tx.wait();
            } else {
                console.log(`第 ${i+1} 个钱包 ${walletiWithProvider.address} ETH 不足以支付 gas，跳过`);
            }
        } catch (e) {
            console.log(`第 ${i+1} 个钱包归集ETH出错:`, e.message);
        }
    }
    ...
    for (let i = 0; i < numWallet; i++) {
        try {
            ...
            // 查看WETH余额
            let balanceWETH = await contractConnected.balanceOf(walletiWithProvider.address);
            if (balanceWETH > 0) {
                let tx = await contractConnected.transfer(wallet.address, balanceWETH);
                console.log(`第 ${i+1} 个钱包 ${walletiWithProvider.address} WETH 归集开始, 归集金额: ${ethers.formatEther(balanceWETH)}`);
                await tx.wait();
            } else {
                console.log(`第 ${i+1} 个钱包 ${walletiWithProvider.address} WETH 余额为0，跳过`);
            }
        } catch (e) {
            console.log(`第 ${i+1} 个钱包归集WETH出错:`, e.message);
        }
    }
