...
// 处理 USDT decimals
function format_usdt(amount_int) {
  return Number(amount_int) / 10 ** 6;
}
...
provider.on('pending', async (txHash) => {
    if (txHash) {
        ...
        j++
        if (tx !== null && tx.data.indexOf(selector) !== -1 && tx.to == "0xdAC17F958D2ee523a2206206994597C13D831ec7") {
            ...
            console.log(`转账金额:${format_usdt(iface.parseTransaction(tx).args[1])}`)
